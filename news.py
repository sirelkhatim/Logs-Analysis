...
query3 = """
            WITH requests AS
            ( SELECT time::date AS day, count(*) FROM log GROUP BY
            time::date ORDER BY time::date),
            errors AS
            ...
            AS error_percentage FROM requests, errors
...
def get_posts(query):
    """
    Function that connects to a database executes a
    query and outputs the result after closing the connection
    :param
...
def get_most_popular_articles():
    """
    Function that formats the output of query
    1 to display it in a user friendly way
    """
...
def get_most_popular_authors():
    """
    Function that formats the output of
    query2 to display it in a user friendly way
    """
    results = get_posts(query2)
    print("Most popular authors:\n")
    for result in results:
        print(
            '{author} - {count} views'.format(author=result[0],
                                              count=result[1]))
    print("\n")
    return
...
def get_errors():
    """
    Function that formats the output of
    query3 to display it in a user friendly way
    """
